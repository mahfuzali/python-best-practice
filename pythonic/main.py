...
def log_food(description: str, calories: int, date: str | None = None) -> None:
    if date is None:
        date = today()
    append_entry(FOOD_CSV, Entry(date=date, description=description, calories=calories))

def log_activity(description: str, calories: int, date: str | None = None) -> None:
    if date is None:
        date = today()
    append_entry(ACTIVITY_CSV, Entry(date=date, description=description, calories=calories))
    ...
    try:
        with path.open() as f:
            for line in f:
                ...
                yield Entry(date=date, description=desc, calories=int(cals))
